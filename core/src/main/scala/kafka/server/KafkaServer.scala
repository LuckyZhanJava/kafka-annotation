  ...
  def startup(): Unit = {
    try {
      ...
      if (canStartup) {
        ...
        /**
         * 从 ZK 创建或获取 ClusterId （base64 处理的 uuid）
         */
        ...
        /* generate brokerId */
        /**
         * 创建 BrokerId （从配置文件中读取，或者利用 ZK 的 /brokers/seqid 节点来获取）
         */
        ...
        /* start scheduler */
        /**
         * 实例化KafkaScheduler对象，给kafka其他需要定时任务使用
         */
        ...
        /**
         * 启动 replica manager ，里面启动了三个定时任务
         * 1.添加 isr-expiration 周期任务，周期: replicaLagTimeMaxMs/2；用于将超过 replicaLagTimeMaxMs 的同步时间的 replica 从 ISR 中移除
         * 2.添加 isr-change-propagation 周期任务，周期: 2500ms；传播 ISR 变动，即创建持久 ZK 节点：/isr_change_notification/isr_change_{sequenceNumber}
         * 3.添加 shutdown-idle-replica-alter-log-dirs-thread 周期任务，周期 10s；关闭多余的线程任务
         *
         * */
        ...
        /**
         *  ZK 节点 /brokers/ids/{id} 注册 Broker 信息
         */
        ...
        /**
         * 创建 BrokerMetadata 的 checkpoint
         */
        ...
        /**
         * 启动 Kafka controller
         * 监听 ControllerEvent [ Startup、IsrChangeNotification、Reelect、BrokerChange、ControllerChange、TopicChange 等]
         * 并启动 controller 选举
         *
         */
        ...
        /**
         * 启动 GroupCoordinator ；启动 delete-expired-group-metadata 周期任务，默认 60s 一次 清除 group 的 offsets
         */
