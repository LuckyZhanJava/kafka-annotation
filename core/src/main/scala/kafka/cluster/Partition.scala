...
class Partition(val topicPartition: TopicPartition,
  ...
  private def maybeIncrementLeaderHW(leaderLog: Log, curTime: Long = time.milliseconds): Boolean = {
    inReadLock(leaderIsrUpdateLock) {
      ...
      // 遍历没有被踢掉的，并且和leader的同步的时间和leader间隔差值小于replicaLagTimeMaxMs
  ...
  private def isFollowerOutOfSync(replicaId: Int,
    ...
    // follower和leader的同步的时间间隔差值大于maxLagMs
    // 注： 0.9 版本之前是判断结束位移(以下皆称LEO, log end offset)的差值(replica.lag.max.messages)，为什么要替换掉这个参数的原因是：
    // leo差值这个参数是全局的！即所有topic都受到这个参数的影响。
    // 假设集群中有两个topic： t1和t2。假设它们的流量差异非常巨大，t1的消息生产者一次性生产5000条消息，直接就突破了4000这个默认值；
    // 而另一个topic，t2，它的消息生产者一次性生产10条消息，那么Kafka就需要相当长的时间才能辨别出t2各个分区中那些滞后的副本。
    // 很显然这种流量差异巨大的topic很容易地在同一个集群上部署，那么这种情况下参数应该设置什么值呢？ 显然没有合适的值
