    ...
    try {
      while (isRunning) {
        try {
          ...
          // 从newConnections队列中拉取channel，注册读事件
          configureNewConnections()
          // register any new responses for writing
          // 从responseQueue队列中拉取channel，注册写事件
          processNewResponses()
          // 调用NetworkReceive.readFrom读取数据，调用Send.send发送响应
          poll()
          // 处理可以读取的请求，放到RequestChannel的RequestQueue队列
